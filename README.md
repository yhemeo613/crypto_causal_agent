...
> ⚖️ **许可证**：[自定义许可证](LICENSE) —— 个人学习/学术研究**免费**；**商业使用需付费授权**（联系作者）。
...
- [1. 我们想干什么](#1-我们想干什么)
- [2. 项目价值](#2-项目价值)
- [3. 核心架构：七层流水线](#3-核心架构七层流水线)
- [4. 十大创新点](#4-十大创新点)
- [5. 全闭环决策周期（Agent 每 2 分钟做什么）](#5-全闭环决策周期agent-每-2-分钟做什么)
- [6. 技术栈](#6-技术栈)
- [7. 目录结构](#7-目录结构)
- [8. 数据库设计（双库 + 图谱 + 向量）](#8-数据库设计双库--图谱--向量)
- [9. 快速开始](#9-快速开始)
- [10. 使用指南](#10-使用指南)
- [11. Dashboard Agent 工作台](#11-dashboard-agent-工作台)
- [12. API 参考](#12-api-参考)
- [13. 实验能力矩阵](#13-实验能力矩阵)
- [14. 配置说明](#14-配置说明)
- [15. 测试](#15-测试)
...
1. **方法论缝合**：将「因果推理 + LLM 多智能体辩论 + 遗传进化」整合进同一可运行框架。现有开源项目多聚焦单一赛道（FinRL 偏强化学习、QLib 偏因子挖掘、TradingAgents 偏对话式多智能体），**同时覆盖因果图谱、证伪推理、双层基因进化、四环境泛化验证**的完整框架稀缺。
2. **可验证的创新**：内置消融框架（7 个创新点独立开关），每个"创新"是否真的有效可通过实验对照证明——这是学术框架区别于 demo 的关键；十大创新点全部有真实落地模块与测试。
3. **反过拟合设计**：四环境（牛/熊/震荡/极端）沙箱 + 泛化惩罚项，正面回应"单一行情过拟合"这一金融 AI 研究的头号批评；收敛性分析、实验版本管理（experiment_id + 对比）支撑可复现论文级实验。

### 工程价值

1. **完整可运行**：真实数据（176 万根 K 线）→ 真实撮合（滑点/手续费/爆仓/强平）→ 真实 LLM（DeepSeek）→ 真实落库（双库 + Neo4j + ChromaDB），86 个测试全绿，非 notebook 演示。
2. **双库架构**：业务数据（决策/进化/复盘）与时序数据（K 线/费率）物理分离——本地 PG 5432 + TimescaleDB 5433，各归其位；Neo4j 存因果图谱、ChromaDB 存向量记忆，五类存储各司其职。
3. **Agent 工作台**：11 页可视化 Dashboard（因果图谱力导向图、专业 K 线、进化 3D 曲面、实时日志流、工具调用审计），50+ 开放 API，把"黑盒 Agent"变成可观察、可操作、可审计的研究工具。
4. **成本友好**：主力数据源免费（Binance/OKX/Bybit + FRED + CoinGecko + Fear&Greed），LLM 可切换低成本 provider，单机 CPU 即可跑完整闭环。

### 定位声明

这是一个**研究 Agent 如何做金融决策的实验室**，而非"能赚钱的交易系统"。它最大的意义在于：把 AI 金融研究从"prompt 调参"提升到**可拆解、可验证、可进化**的工程科学层面。框架的方法论价值独立于其策略收益——收益能力未证实，但研究价值真实存在。
